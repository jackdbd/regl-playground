...
const drawDots = regl({
  ...
  attributes: {
    xyPixelPosition: (context, props) => {
      ...
      const x1 =
        halfWidth + halfWidth * props.sineWaveFunction({ t: context.time });
      const x2 =
        halfWidth +
        halfWidth * props.sineWaveFunction({ t: context.time, amplitude: 0.5 });
      const x3 =
        halfWidth +
        halfWidth * props.sineWaveFunction({ t: context.time, tau: 2 });
      const y4 =
        halfHeight +
        halfHeight * props.sineWaveFunction({ t: context.time, tau: 5 });
